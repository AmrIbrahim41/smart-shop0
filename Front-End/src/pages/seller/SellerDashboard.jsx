...
const SellerDashboard = () => {
  ...
  const fetchMyProducts = useCallback(async (page = 1) => {
    ...
    setError(null);
    try {
      const response = await api.get(`${ENDPOINTS.MY_PRODUCTS}?page=${page}`);
      const data = response.data;

      // Handle both plain array (legacy) and paginated envelope
  ...
  // NOTE: intentionally depend only on currentPage; fetchMyProducts is stable
    ...
    return (
      <div className="flex justify-center items-center gap-3 mt-8">
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2.5 text-sm font-bold bg-white dark:bg-gray-800 border border-gray-200 dark:border-white/10 rounded-xl text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-40 transition-colors"
        >
  ...
  return (
    <div className="min-h-screen pt-28 lg:pt-32 pb-10 px-4 md:px-6 bg-gray-50 dark:bg-gray-900 transition-colors duration-500 animate-fade-in-up">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1 font-medium">
              {t('manageCatalog') || "Manage your catalog and stock"}
              {totalProducts > 0 && (
                <span className="ml-2 px-2 py-0.5 bg-primary/10 text-primary rounded-lg font-black text-xs">
        ...
        {/* Error State */}
        {error && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 px-6 py-4 rounded-2xl mb-8 flex items-center gap-3 font-bold">
        ...
        {!loading && !error && products.length === 0 && (
          <div className="text-center py-24">
            <FaBoxOpen className="text-7xl text-gray-300 dark:text-gray-700 mx-auto mb-5" />
            <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-2">
              {t('noProducts') || "No products yet"}
            </h2>
            <p className="text-gray-500 dark:text-gray-400 mb-6 font-medium">
              {t('startAdding') || "Start adding products to your store now."}
            </p>
            <Link to="/seller/products/add" className="text-primary font-bold hover:underline">
              {t('startSelling') || "Start Selling"} &rarr;
            </Link>
          </div>
        )}
                ...
                {products.map((product) => {
                  ...
                  return (
                    <div
                      ...
                      className="bg-white dark:bg-gray-800 p-4 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-lg transition-all duration-300 group flex flex-col"
                    >
                      ...
                      <div className="flex justify-between items-start mb-2 gap-2">
                        <h3
                          className="font-bold text-gray-900 dark:text-white truncate flex-1 text-sm md:text-base"
                          title={product.name}
                        >
                          {product.name}
                        </h3>
                      ...
                      <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-4 font-medium bg-gray-50 dark:bg-white/5 px-3 py-2 rounded-lg mt-auto">
                        <span>
                          {t('stock') || "Stock"}:{' '}
                          <b className={stockCount > 0 ? 'text-green-500' : 'text-red-500'}>
                            {stockCount}
                          </b>
                        </span>
                        <span className="truncate max-w-[80px]" title={product.brand}>
                          {product.brand || "-"}
                        </span>
                      </div>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                      {products.map((product) => {
                        ...
                        return (
                          <tr
                            key={product.id || product._id}
                            className="hover:bg-gray-50 dark:hover:bg-white/5 transition duration-200"
                          >
                            <td className="p-5 pl-6">
                              <div className="flex items-center gap-4">
                                ...
                                <div className="flex flex-col">
                                  <span
                                    className="font-bold text-gray-900 dark:text-white truncate max-w-[150px] md:max-w-xs"
                                    title={product.name}
                                  >
                                    {product.name}
                                  </span>
                            ...
                            <td className="p-5 text-right pr-6">
                              <div className="flex items-center justify-end gap-2">
                                <Link
                                  to={`/product/${product.id || product._id}`}
                                  className="p-2 bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-white/20 transition"
                                  title={t('view') || "View"}
                                >
                                  <FaEye />
                                </Link>
